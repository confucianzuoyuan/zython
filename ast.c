...
static inline char *bufferNewCString(size_t length) {
  ...
  if (buffer == NULL) {
    fprintf(stderr, "Not enough memory to allocate string of length %zu.",
            length);
    exit(1);
  }
  return buffer;
...
char *tokenToCString(ZyToken token) {
...
void AstArrayAdd(AstArray *buffer, Ast *element) {
  if (buffer->capacity < buffer->count + 1) {
    ...
    Ast **elements =
        (Ast **)realloc(buffer->elements, sizeof(Ast *) * buffer->capacity);
    if (elements != NULL)
...
void AstArrayAddAll(AstArray *from, AstArray *to) {
  ...
  for (int i = 0; i < from->count; i++) {
    AstArrayAdd(to, from->elements[i]);
  }
}

int AstArrayFirstIndex(AstArray *buffer, Ast *element) {
  for (int i = 0; i < buffer->count; i++) {
    if (buffer->elements[i] == element)
      return i;
  }
  return -1;
}
...
Ast *AstArrayDelete(AstArray *buffer, int index) {
  Ast *element = buffer->elements[index];
...
void freeAst(Ast *ast, bool freeChildren) {
  if (ast->children != NULL) {
    if (freeChildren)
      freeAstChildren(ast->children, freeChildren);
